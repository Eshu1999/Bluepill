...
import { Camera, Save, User as UserIcon, Users, Heart, Plus, AtSign, ArrowLeft, ShieldQuestion, QrCode } from 'lucide-react';
...
export function ProfileView({ profile, user }: ProfileViewProps) {
  ...
  const form = useForm<ProfileFormValues>({
    ...
    defaultValues: {
      ...
      pictureUrl: profile?.photoURL || user?.photoURL || '',
    },
  });
  
  const getQrCodeData = () => {
    if (!profile || !user) return JSON.stringify({ error: "Profile not loaded" });
    return JSON.stringify({
        userId: user.uid,
        accountType: profile.accountType || 'normal',
    });
  }
                                     ...
                                     <DialogContent>
                                        <DialogHeader>
                                            <DialogTitle>Your Personal QR Code</DialogTitle>
                                             <DialogDescription>
                                                {profile?.accountType === 'chemist' 
                                                    ? "Have customers scan this to send you medication requests."
                                                    : "Have a family member scan this to instantly connect."
                                                }
                                             </DialogDescription>
                                        </DialogHeader>
